...
public class ExcelController {
     ...
     * @throws IOException
     */
    @RequestMapping(value = "Charge/{beH_id}", method = RequestMethod.GET)
    public void excelCharge(HttpServletResponse response,@PathVariable("beH_id") Integer beH_id) throws IOException {
        response.setCharacterEncoding("UTF-8");
        List<ExcelChargeProjVo> projVos = excelService.queryChargeInfo(beH_id);
        // 创建Excel文件
        HSSFWorkbook wb = new HSSFWorkbook();
        // 创建Sheet页
        HSSFSheet sheet = wb.createSheet("收费明细表");
        // 创建标题行
        HSSFRow titleRow = sheet.createRow(0);
        titleRow.createCell(0).setCellValue("病例号");
        titleRow.createCell(1).setCellValue("姓名");
        titleRow.createCell(2).setCellValue("收费项目");
        titleRow.createCell(3).setCellValue("收费金额");
        titleRow.createCell(4).setCellValue("检查日期");
        // 遍历将数据放到Excel列中
        String state = "";
        for (ExcelChargeProjVo vo : projVos) {
            HSSFRow dataRow = sheet.createRow(sheet.getLastRowNum() + 1);
            dataRow.createCell(0).setCellValue(vo.getBeH_id());
            dataRow.createCell(1).setCellValue(vo.getRe_name());
            dataRow.createCell(2).setCellValue(vo.getCharP_name());
            dataRow.createCell(3).setCellValue("¥ " + vo.getCharP_money());
            dataRow.createCell(4).setCellValue(vo.getCheckTime().toString());
        }
